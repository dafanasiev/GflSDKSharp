...
namespace GflSDK.Interop
{
    ...
    internal sealed class UnmanagedLibrary : IDisposable
    {
        ...
        public UnmanagedLibrary(string fileName)
        {
            ...
            _resourceFileName = "GflSDK.lib." + Platform.PlatformName + "." + CurrentArch + "." + fileName + Platform.LibSuffix;
            _extractedFileName = fileName + "-" + CurrentArch + Platform.LibSuffix;
            ...
            if (_handle == null || _handle.IsInvalid)
            {
                throw new FileNotFoundException(
                    "Unable to find " + _extractedFileName + " on system path or the file found was not the expected file.",
                    _extractedFileName,
        ...
        private SafeLibraryHandle LoadFromSystemPath()
        {
            ...
            return NullifyInvalidHandle(Platform.OpenHandle(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), _extractedFileName)))        //near executing assembly
                ?? NullifyInvalidHandle(Platform.OpenHandle(_extractedFileName));  //in system paths
        ...
        private SafeLibraryHandle ExtractAndLoadFromPath(string dir)
        {
            ...
            if (!ManifestResource.Extract(_resourceFileName, libPath))
            {
                return null;
            }
